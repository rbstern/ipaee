...
o valor de $500$, isto é, 
$\bar{X}-500$. Gostaríamos de rejeitar
a hipótese nula quando $\bar{X}$ é muito maior que $500$,
isto é, $\bar{X}-500 > c$, onde $c$ é 
...
queremos calcular $P(\bar{X} - 500 > c)$ sob $H_0$.
Especificamente, gostaríamos que 
$P(\bar{X} - 500 > c) \leq \alpha$ sob $H_0$.
Para realizar esta desigualdade, note que
decorre das propriedades da distribuição normal que, 
sob o valor extremo o extremo da hipótese nula ($\mu = 500$),
temos que $\bar{X}-500 \sim N\left(0,\frac{\sigma_0^2}{n}\right)$.
Assim, utilizando a padronização da distribuição normal, obtemos que
se $\mu = 500$,
$$
 Z := \frac{\bar{X}-500}{\sqrt{\frac{\sigma_0^2}{n}}} \sim N(0, 1)
$$
Portanto,
$$
\begin{align*}
 P(\bar{X}-500 > c)
 &= P\left(\frac{\bar{X}-500}{\sqrt{\frac{\sigma_0^2}{n}}} > \frac{\sqrt{n}c}{\sigma_0}\right) \\
...
rejeitamos a a hipótese nula $H_0:\mu \leq 500$ quando
$$\bar{X}-500 > \frac{\text{qnorm}(1-\alpha)\sigma_0}{\sqrt{n}}.$$
...
2. Considere o caso da normal com variância conhecida.
